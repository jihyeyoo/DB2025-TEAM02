...
public class MyStudyDAO {
    ...
    // 스터디 목록 조회 (loginId 기준)
    public List<MyStudyDTO> getMyStudies(String loginId) {
        ...
        String sql = "SELECT sg.study_id, sg.name AS study_name, u.user_name AS leader_name, sg.start_date " +
                     ...
                     "WHERE gm.user_id = (SELECT user_id FROM Users WHERE login_id = ?)";
    ...
    // 탈퇴 기능 (loginId 기준)
    public boolean withdrawFromStudy(int studyId, String loginId) {
        String sql = "DELETE FROM GroupMembers WHERE study_id = ? AND user_id = (SELECT user_id FROM Users WHERE login_id = ?)";
        try (PreparedStatement pstmt = AppMain.conn.prepareStatement(sql)) {
